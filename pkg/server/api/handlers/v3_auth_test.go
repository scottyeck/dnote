...
// 			authKey:      "SLMZFM5RmSjA5vfXnG5lPOnrpZSbtmV76cnAcrlr2yU",
// 			cipherKeyEnc: "someRandomCipherKeyEncrypted1",
// 			iteration:    10000,
// 		},
// 		{
// 			email:        "bob@example.com",
// 			authKey:      "CQYOJ4NVPIyv7m8VWWC1ILnj6C+vDeTzJb2ZpRZX5Mo=",
// 			cipherKeyEnc: "someRandomCipherKeyEncrypted2",
// 			iteration:    20000,
// 		},
// 		{
// 			email:        "chuck@example.com",
// 			authKey:      "LRczMQcnIPrcGZkATjSurvyVjKvzm+sVVFh1MK1PwBM=",
// 			cipherKeyEnc: "someRandomCipherKeyEncrypted3",
// 			iteration:    15000,
// 		},
// 	}
//
// 	for _, tc := range testCases {
// 		t.Run(fmt.Sprintf("register %s %s %d", tc.email, tc.authKey, tc.iteration), func(t *testing.T) {
...
// 			defer server.Close()
//
// 			dat := fmt.Sprintf(`{"email": "%s", "auth_key": "%s", "iteration": %d, "cipher_key_enc": "%s"}`, tc.email, tc.authKey, tc.iteration, tc.cipherKeyEnc)
...
// 			assert.NotEqual(t, account.Salt, "", "Salt mismatch")
// 			assert.Equal(t, account.ServerKDFIteration, crypt.ServerKDFIteration, "ServerKDFIteration mismatch")
// 			assert.Equal(t, account.AuthKeyHash, crypt.HashAuthKey(tc.authKey, account.Salt, account.ServerKDFIteration), "AuthKeyHash mismatch")
// 			assert.Equal(t, account.ClientKDFIteration, tc.iteration, "ClientKDFIteration mismatch")
// 			assert.Equal(t, account.EmailVerified, false, "EmailVerified mismatch")
// 			assert.Equal(t, account.CipherKeyEnc, tc.cipherKeyEnc, "CipherKeyEnc mismatch")
...
// 		defer server.Close()
//
// 		dat := fmt.Sprintf(`{"auth_key": "%s", "iteration": %d}`, "SLMZFM5RmSjA5vfXnG5lPOnrpZSbtmV76cnAcrlr2yU", 10000)
// 		req := testutils.MakeReq(server, "POST", "/v3/register", dat)
//
// 		// Execute
// 		res := testutils.HTTPDo(t, req)
...
// 		defer server.Close()
//
// 		dat := fmt.Sprintf(`{"email": "%s", "iteration": %d}`, "alice@example.com", 10000)
// 		req := testutils.MakeReq(server, "POST", "/v3/register", dat)
//
// 		// Execute
// 		res := testutils.HTTPDo(t, req)
...
// 		defer server.Close()
//
// 		dat := fmt.Sprintf(`{"email": "%s", "auth_key": "%s"}`, "alice@example.com", "SLMZFM5RmSjA5vfXnG5lPOnrpZSbtmV76cnAcrlr2yU")
// 		req := testutils.MakeReq(server, "POST", "/v3/register", dat)
//
// 		// Execute
// 		res := testutils.HTTPDo(t, req)
